...
from torch.optim import NAdam, AdamW, Optimizer,Adam
    ...
    if current_args is not None:
        try:
            if current_args.pred_max_atom_size is not None and current_args.pred_max_atom_size != args.max_atom_size:  # prediction process
                debug(
                    f'{path}: generalization prediction, original max_atom_size: {args.max_atom_size}, now predicting max_atom_size = {current_args.pred_max_atom_size} molecule.')
            else:
                debug(
                    f'{path}: prediction process, but without generalization, use the original max_atom_size: {args.max_atom_size}')
            args = current_args
            args.max_atom_size = current_args.pred_max_atom_size
        except:
            if current_args.max_atom_size is not None:
                debug(
                    f'transfer learning: change max_atom_size from {args.max_atom_size} to {current_args.max_atom_size}')
...
def get_loss_func(args: Namespace) -> nn.Module:
 ...
 """
    ...
    ### For mixed model ###
    if args.dataset_type == 'regression' and args.aleatoric and (
            args.fp_method in ['atomic', 'hybrid_dim0', 'hybrid_dim1']):
...
def create_logger_atl_all(name: str, save_dir: str = None, quiet: bool = False,
                          active_iter: int = -1) -> logging.Logger:
    """
